# Practice篇: iOSProject Development Skills
...
- [Theory篇: Swift/ObjC Language Fundamentals](https://github.com/ChenYilong/iOSInterviewQuestions/blob/master/02_Swift_interview_questions/theory.md) 
 - [Practice篇: iOSProject Development Skills]( https://github.com/ChenYilong/iOSInterviewQuestions/blob/master/02_Swift_interview_questions/practical.md ) 
 ...
 **Combine、Functional and Reactive Programming:**

- Explain reactive programming and its advantages, mentioning frameworks like RxSwift and Combine. How to Swift implement functional programming in?Please give an example.
- Q.Swift 是Object-Oriented Programmingor is it a functional programming language?
...
**ConcurrencyandMulti-threaded Development:**
...
- StructuredConcurrency（Structured Concurrency）:How to implement structured concurrency?
...
**Debugging、VersionManagement、Project Management:**
...
iOS17 after 是ObservationFramework,性能更好了,不过写法不同了.  [SwiftUI Data Flow 2023]( https://troz.net/post/2023/swiftui-data-flow-2023/ "") 
...
Swift 既是Object-Oriented Programming的,又是function式的编程语言.
说 Swift 是Object-Oriented Programming的语言,是因为 Swift 支持类的Encapsulation、继承、andPolymorphism,从这点上来看与 Java 这类纯Object-Oriented Programming的语言几乎毫无差别.
说 Swift 是function式编程语言,是因为 Swift 支持 map, reduce, filter, flatmap 这类去除中间状态、数学function式的method,更加强调运算结果而不是中间过程.

function式的编程,通俗易懂的讲解

[一文读懂Swiftfunction式编程](https://zhuanlan.zhihu.com/p/192483039)

拓展问题: How to Swift implement functional programming in?Please give an example.
...
is那个TCA?TCA在clean的基础上加了很多功能. 很多名称在TCAandclean上叫的不一样.

而且tca组件之间通讯有点复杂.

复杂页面的话. 冗余代码太多, 比如usercaseandservice
...
TCA(TCA一般是指:the composable architecture)有点插件式的感觉,你要调用子component的逻辑,你就得把它的state、action、scope都Declare一下就能用了.

有的, 
TCA 虽然是一种Design Patterns, 但也有一个非常流行的最佳实践库, 可以理解为 [TCA 标准库: The Composable Architecture]( https://github.com/pointfreeco/swift-composable-architecture "") 
感觉最大的优势,还是代码清晰....

state,action本身内部内聚很高, 类似Declare. 一旦类似delegate的action出去了. 单看一个模块就读不出完整逻辑了. 我觉得reducer做到了有所有的逻辑啊,delegate也只是一种effect而已.  跟Other的effect没有什么区别. 原来这个模块这条链路是这个意思.它问了CleanFramework, 我说我听过, 没用过. 新西兰有很多移动端的架构isClean.
...
在MVVM（Model-View-ViewModel）架构中,ViewModel 是一个抽象,它代表了 View 需要用到的数据and命令.因此,ViewModel 通常会包含以下类型的代码:

1. **数据绑定**:ViewModel 包含了视图需要的所有数据,这些数据通常会通过数据绑定的方式在视图中显示.因此,ViewModel 通常会包含一些property（property）,这些property被绑定到视图上,当property的值改变时,视图会自动更新.
2. **命令**:ViewModel 还会包含一些命令（Command）,这些命令对应了用户的操作（比如点击按钮）.命令通常会修改 ViewModel in数据,然后这些修改会通过数据绑定反映到视图上.
3. **数据访问and业务逻辑**:ViewModel 还可能包含一些业务逻辑,比如数据的读取and写入.这些业务逻辑可能会通过调用 Model 层的method来实现.
4. **状态Management**:ViewModel 还可能包含一些状态Management的代码,比如正在加载的状态、错误状态等.
...
需要注意的是,ViewModel 不应该直接访问 View,所有的交互都应该通过数据绑定and命令来完成.这样可以确保 ViewModel and View 的解耦,使得 ViewModel 可以方便的进行单元测试.

```swift
//下面是一段 MVVM in ViewModel 代码. 请指出下面的代码中, 分别对应着 ViewModel 的哪些职责?
...
1. **数据绑定**:在这个例子中,`contents`,`contentCellViewModels`,`viewState` and `searchText` 都是 `Observable` 类型,它们都被绑定到了视图上.当这些 `Observable` object的值改变时,视图会自动更新.
2. **命令**:`func contentCellViewModel(for content: Content) -> ContentCellViewModel` and `func update() async throws` 这两个methodis命令,它们对应了用户的操作,比如点击按钮或者刷新页面.
3. **数据访问and业务逻辑**:在 `func update() async throws` method中,代码调用了 `repository.fetchPosts()` method从服务器获取数据,这部分is数据访问.然后在获取数据后,代码根据获取的数据修改了 `contents` and `allPosts`,这部分is业务逻辑.
4. **状态Management**:`viewState` propertyis用来Management状态的.它的值可能是 `.loading`,`.loaded` 或者 `.error`,分别代表了不同的状态.在 `func update() async throws` method中,根据数据加载的结果,代码会修改 `viewState` 的值,从而反映出当前的状态.
...
最初的MVC解决了 View and Model 的角色分离, and View Action and Model Action 的处理问题... 后续的七七八八的架构产生都是为了测试目的...

国内其实除了MVVM, Other的都不care, 毕竟衍生的架构都是为了test. 而我们不需要test...
...
viperis过度设计的mvp. 其实is比标准的mvp多了个interactor. 
viper里E指的其实ismodel,非得改个名字.R是router,感觉跟mvvm-c里的coordinate却差不多.
interactor感觉is为了搞一个没有任何ui相关逻辑的可注入层.
...
依赖注入（Dependency Injection）,一种常用的软件Design Patterns,广泛应用于许多开发环境and语言中.重点在于低耦合. 

#### 1. iOS上的依赖注入

在iOS开发中,依赖注入可能通过视图控制器的constructionmethod来实现.与Spring的@Autowired resourcecompared to,虽然在某些方面存在异同,但核心目标依然是达到程序的低耦合.

#### 2. 依赖注入的理解层次

依赖注入并非一个独立的概念,它涵盖了constructionfunction注入、Settermethod注入等多种实现手段.一些开发人员可能会将其与Spring的特性相联系,但这种理解可能并不全面.更精确的理解应该强调组件间依赖的外部实现.
...
为每个 view controller 上都使用了一个 context property,并要求在 view controller 构建的时候传入这个上下文property.虽然and基础 MVC 中设定一个 model objectcompared to没有特别大的不同,但是它还是展示了一种避免单例的方式.这也是依赖注入降低耦合的实践.

实践2:

如果想在 view controller 上设置一个初始 model 值时,我们有不同的方式:

方式一, 通过判定 controller 在 controller 层级上的位置以及 controller 的类型,直接访问一个全局的 model object.
方式二, 开始时将 model object的引用设置为 nil 并让所有东西保持为空白状态,直到另一个 controller 提供了一个非 nil 值.
第三种选择是在 controller 初始化时将 model object当作parameter传递进来 (也is依赖注入).
...
对于function`ft`来说,它只知道类A,subclassSA是外部注入的,它也只能依赖A的特征and行为.


#### 4. 高层次的理解and扩展

通过将依赖注入描述为强依赖转变为弱依赖,可以深刻地揭示其背后的设计原则,如面向接口编程.此外,SwiftUI的 `Environtment`、`EnvironmentObject` 也可以看做是依赖注入的一种扩展实践.for example,这里有一个SwiftUI的依赖注入库: [swift-dependencies](https://github.com/pointfreeco/swift-dependencies).

从Theory上讲,依赖注入是一种实现依赖反转原则的技术.该原则指出,高级模块不应依赖于低级模块,而是两者都应依赖于抽象模块.

在 iOS 应用程序中,依赖注入意味着依赖于另一个object（如依赖于模型object的视图控制器）的object将获得对其依赖object的引用,而不是创建object本身.这使得依赖于Otherobject的object更加灵活,也更易于测试,因为在测试过程中它可以获得一个模拟object来使用.
...
下面是一个在 iOS 应用程序中使用 SwiftUI and Combine Framework进行 DI（依赖注入）的简单示例:
...
这使得 UserViewModel 更加灵活,因为它可以获得任何符合 NetworkManager 协议的object,而不仅仅是 NetworkManager 的特定实现.这使得测试 UserViewModel 更加容易,因为在测试过程中可以注入一个模拟object.

现在,我们将尝试把 "简洁架构 "and "依赖注入 "这两种method结合在一起,利用前面示例in知识来获得一个完整清晰的观点.



### 总结

依赖注入作为一个重要的Design Patterns,不仅在具体的实现细节上有着丰富的内容,而且在更高层次的设计原则上也有深远的意义.通过多角度、多平台的讨论,我们不仅能够对依赖注入的具体实现有更准确的理解,还可以洞察其背后所蕴藏的软件设计智慧.

讨论中涉及的观点,如依赖注入的核心目标:低耦合,property注入and Java in用法,插件化开发与依赖注入的联系,以及与Polymorphism的关系等,都为我们提供了丰富的视野and启示.
...
我回答的, 是用GCD, 重写 getterandsetter, 实现异步读取, 同步写入.只要保证setterandgetter在一个队列里就行. setter是同步, getter同步and异步都行.
...
一个actormethod不能返回一个值、抛出一个error或拥有一个inout的parameter
所有的parameter需要在被拷贝时生成独立的值 
本地状态and非actormethod只能被语法上定义在Actor中,或其 extension之inmethod所访问.

 [Swift Concurrency Manifesto]( https://gist.github.com/lattner/31ed37682ef1576b16bca1432ea9f782 "") 
 [Swift Concurrency宣言]( https://gist.github.com/kkpan11/49e14f23f00bd3e8d7c0853c9c0d55bb#%E7%AC%AC%E4%BA%8C%E9%83%A8%E5%88%86%EF%BC%9AActor%EF%BC%9A%E6%B6%88%E7%81%AD%E5%85%B1%E4%BA%AB%E5%8F%AF%E5%8F%98%E7%8A%B6%E6%80%81 ) 
...
这个题is给Task.group量身定制的么. 过去可能得用Semaphore吧.
...
在 Swift 5.5 及after的Version,可以使用 `async/await` 以及 `TaskGroup` 来实现你所说的多个请求同时发出,然后等待所有的结果都回来.以下是一个基本的示例:
![3](assets/3.jpg)
...
在这个示例中,`fetchSomething(id:)` 是一个模拟的异步网络请求function.它接受一个 `id` parameter,然后在一段随机的时间后返回这个 `id`.
...
注意,这个代码应该在某个 `async` function或者Closures中运行.在这个示例中,我们使用 `async {}` 创建了一个异步Closures来运行这段代码.如果你在你自己的代码中使用这个模式,你应该把这段代码放在一个 `async` function中.

最后,请注意处理可能会抛出的错误.在这个示例中,我们使用 `do-catch` 来捕获并打印可能会发生的错误.

在 Swift 中,如果你不使用 async/await,你可以使用 `DispatchGroup` 来Management多个Concurrency的任务.以下是一个基本的示例:
...
在这个示例中,`fetchSomething(id:completion:)` 是一个模拟的异步网络请求function.它接受一个 `id` parameterand一个完成回调,然后在一段随机的时间后调用这个回调并传入这个 `id`.

然后,我们创建一个 `DispatchGroup`,并向这个调度组添加5个异步任务.每个任务都会调用 `fetchSomething(id:completion:)`,并传入不同的 `id` and一个完成回调.

在启动每个任务之前,我们调用 `group.enter()`.在每个任务完成后,我们在完成回调中调用 `group.leave()`.

最后,我们使用 `group.notify(queue:)` 来等待所有的结果.当所有的结果都返回时,这个回调会被调用.

如果你不使用 `DispatchGroup` and `async/await`,你可以使用 `OperationQueue` and `Operation`.你可以将每个网络请求Encapsulation为一个 `Operation`,并将这些 `Operation` 添加到一个 `OperationQueue` 中.然后,你可以创建一个完成操作,设置其依赖为所有的网络请求操作,以此来实现等待所有结果都返回的功能.
![2](assets/2.jpg)
...
在这个示例中,`FetchOperation` 是一个自定义的 `Operation`,用于模拟网络请求.它有一个 `id` property来标识每个请求,以及一个 `result` property来存储每个请求的结果.

然后,我们创建一个 `OperationQueue`,并向这个队列添加5个 `FetchOperation`.每个 `FetchOperation` 都会进行一个网络请求,并将结果存储在其 `result` in properties.
...
如果你不使用 `DispatchGroup`,`async/await`,and `OperationQueue`,可以使用 `Semaphores` 一起配合 `DispatchQueue` 来实现等待所有结果返回.以下是一个基本的示例:
...
在这个示例中,首先创建一个并行队列 `dispatchQueue` and一个信号量 `dispatchSemaphore`.信号量的初始值为0,表示没有可以使用的资源.
...
然后,我们使用一个循环来等待所有的结果.在每次迭代中,我们都调用 `dispatchSemaphore.wait()` 来减少信号量的值,表示我们正在等待一个资源.当所有的资源都变为可用时（也is所有的网络请求都完成时）,这个循环就会结束.
...
注意:这个method仅仅在自己创建的并行队列中有效,对于系统的全局并行队列并不起作用,因为全局并行队列不能被单个任务阻塞.
...
iOS interview问题: 你能解释一下基于令牌的身份验证是如何工作的吗?How to这种身份验证中,从应用程序接口获取一个令牌,并在随后的应用程序接口调用中使用它?

token-based authentication == JWT.
该问题主要是examinationJWT工作原理.
...
1. **用户登录**:用户向服务器提交其凭证（通常是用户名and密码）.

2. **令牌生成**:如果凭证有效,服务器将创建一个令牌.这个令牌可以是一个简单的随机字符串,一个JSON Web Token（JWT）,或者Other形式的令牌,这取决于系统设计.通常,令牌包含或与用户信息相关,可能包含他们在系统in权限或角色.

3. **令牌传送**:服务器将生成的令牌发送回客户端.

4. **客户端存储**:客户端应用在本地存储令牌,for example在web应用的本地存储、会话存储或一个cookie中,或在移动应用的一个安全存储区域.
...
7. **过期and刷新**:令牌具有有限的生命周期,并会在一段时间后过期.这是一个安全功能,用来限制令牌被盗的损害.当令牌过期时,服务器将拒绝请求,客户端应用将需要获取新的令牌.这可以通过重新验证用户或使用"刷新令牌"来完成,刷新令牌是一种生命周期更长的令牌,可以用来换取新的访问令牌.

这个系统使服务器避免存储会话状态数据 - 代替的是,所有必要things都打包到令牌中,从而形成一个无状态的,可扩展的系统.同时,它提供了一个健壮and安全的用户认证method.
...
在Swift代码中安全存储API密钥的method有几种:

- 使用环境变量:你可以在编译时设置环境变量来存储API密钥.这种method的优点是,即使源代码被公开,只有可以访问编译环境的人才能看到密钥.
- 使用`.xcconfig`文件:可以将API密钥存储在`.xcconfig`文件中,并在项目设置中引用该文件.然后在代码中使用`Bundle.main.infoDictionary?["API_KEY"]`来访问API密钥.为了安全,`.xcconfig`文件不应该被包含在Version控制系统中.
- 使用苹果的Keychain服务:Keychain是一个安全的存储系统,可以用来存储各种敏感信息,for example密码、密钥and证书等.
- 使用第三方解决方案:for example,Cocoapods-keys是一个插件,可以让你安全地存储环境变量.

[Secret Management on iOS](https://nshipster.com/secrets/)

是的,我在工作中使用过需要API密钥的API调用.我通常使用以上提到的几种method来安全地存储这些密钥.实际选择哪种method取决于具体的项目需求and团队协议.

当然,以下是一些关于如何改进在Swift中安全存储API密钥的建议:

1. 使用环境变量:确保您的构建环境safety,for example,使用有足够权限的用户帐户来运行构建,并限制对构建服务器的访问.
2. 使用.xcconfig文件:在保存这种类型的文件时,您可以将它们加密,以便即使它们被复制或泄露,也无法直接访问.您还可以使用CI/CD工具,for exampleJenkins或Gitlab CI,在构建时生成这些文件,并在构建完成后立即删除它们.
3. 使用苹果的Keychain服务:您可以使用高级的加密算法（for exampleAES）对保存在Keychainin数据进行加密,以防止被静态分析.
4. 使用第三方解决方案:在选择第三方库时,应充分调查其safety,并确保它们已通过适当的安全审查.您还应该定期更新这些库,以获取任何可能的安全更新.
5. 零知识证明:这是一种先进的密码学技术,可以使服务器验证用户的身份,而无需知道用户的凭证.这种method可以用来替代传统的API密钥,进一步提高应用的safety.
6. 在客户端不存储密钥:最好的method可能是不在客户端存储密钥,而是在服务器端进行存储,并通过安全的method在需要的时候将其传输到客户端.for example,每次需要API密钥时,都可以向一个安全的服务器端点发送请求,然后服务器会返回一个临时密钥,这个密钥在一段时间后会过期.这种method的优点是密钥不会被永久存储在客户端,从而减少了被攻击者获取的可能性.


上面是标准的Answers, 下面谈一下之前在项目in一些实际操作: 
...
比如现在告诉你, apple key id 是 "abcde" 让你存本地, 你会怎么存, 打包就会带上的, 转成ASCII,然后当string用.转成ascii的话. 如果我是想偷key的人, 我就会用反编译工具简单查看所有string.找到这个之后. 打眼一看isascii的. 无任何安全措施. 

it depends,一方面是代码里面可以加密解密. 增加反编译的难度. 另一方面, 可以讲这些key存放在本地的一个文件中, for example你可以把信息放进一个image中, 通过读取image, 再转换成string. 因为没人会知道一张图片是一个存放key的地方, 就算知道了, 也不知道key存在image哪里.让我想起了当年贴吧上的种图. 把片子的种子文件放在一个.rar文件里,然后跟一个image合并,上传贴吧 阿里的组件, 接入流程里, 会告诉你需要安全图片, 实际上is存放 Key的地方. [https://baichuan.taobao.com/docs/doc.htm?spm=a3c0d.7662649.0.0.5608be48IxwIEA&treeId=129&articleId=118101&docType=1](https://baichuan.taobao.com/docs/doc.htm?spm=a3c0d.7662649.0.0.5608be48IxwIEA&treeId=129&articleId=118101&docType=1) 

因为key高概率都是字母数字符号啥的. 基本都在0x7F内.  传统加密会遇到一些问题.你以为要加密, 所以要提供另一个key, 会引入新的安全问题. 但是当然key多了, 估计偷的人也傻逼了.hash一下呢. hash不可逆哦. 我们密码学课上老师讲过,没有破解不了的加密方式,但是你可以让破解成本趋近于无穷. hash之后, 偷的人不知道了, app自己也不知道了. base64也算一种hash. base64结构太典型, 一眼就能看出. 直接decode. Theory上md5都是可逆的....

这个我们is用的keystore而已,外加一个过期时间.这个策略也跟后台他们对token的验证策略有关. 

keystore的问题在于是明文存储.所以需要加密之后塞进去才安全.DSA一下呗.不过密钥怎么存.你这样会引入新的安全问题.当然, 只要你引入的多了. 一堆key 属于是狡兔三窟. 偷的人估计放弃了. 过去一直播的数据加密,也是RSA+DES. 所以我选择使用传替换逻辑. 数据非对称加密,再把密钥对称加密. 简单的话. 替换逻辑最快.偷的人需要知道string 以及变更逻辑.这个都不需要AI就能分析出来....这个变更逻辑is加密算法 (只不过这个加密算法不需要Otherkey, 只是替换. 模仿游戏看过么.凯撒密码?图灵那个年代都能给你解出来.

[Secret Management on iOS](https://nshipster.com/secrets/)

举个例子嘛, is需要一个不需要Otherkey的加密算法. 需要一本莫名其妙的小说,当密码母本是吧.你之前说的base64, 简单ascii, 破解起来太轻松了.我们能做的is提高他们的破解门槛.他那台机子可不是一般人能有的.机械密码发展到最后也is英格玛密码机了,40年代的算力都能破解. 他能破解我, 但是懒得破解我, 所以我是安全的.
...
加点盐就够了.这是纯本地的嘛.appkey一般都是纯本地加密吧.你说的是oauth那套么.能放server不放client.如果只是判断用户身份的话. appkey没有啥好保密的. appsecret是关键, 是加密key.保护appsecret的methodis上面咱们一直讨论things.app launch的时候就需要appkey呢?.放server的话,来不及啊.比如推送的key.看需求, 你说的是三方的appsecret是吧.我们之前用的推送库,既有appkey,也有appsecret.不可能放server啊,因为launch的时候,就需要验证这俩了.他们应该只需要传appkey吧.我看分享的那些只要appkey.不是,两个都得传.一个很奇怪的platform.以前用firebase,就只需要传一个.哦哦, Weibois两个都穿.那确实奇怪. 为啥要穿这个. 可能Weibo用的不是Theory上的appsecret ?现在呢. 我们换推送平台了.就变2个了
...
关于Xcode新Version的查看 View Memory的步骤如下: To view memory in Xcode, go to Debug → Debug Workflow → View Memory.
...
- 使用最新Version的 macOS 虚拟机.
- 使用 Ruby Version >= 2.6.
- 安装 Cocoapods 并切换到指定的 Xcode Version.
- 执行 `swift build` and `swift test`.

 保存并运行 Pipeline.

请注意,以上只是一个基本示例,实际的 CI/CD 设置将根据你的项目的具体需求有所不同.

同时,由于 Swift 的开发一般依赖于 Xcode,而 Xcode 目前只能在 macOS 上运行,所以在 Azure Pipelines 上使用 macOS 环境进行构建and测试需要有一定的付费成本.具体的费用请参考 Azure 的官方定价信息.
...
下面是一个基本的 GitHub Action 的 YAML 文件示例,用于在一个简单项目上运行连续Integration过程:
...
在这个例子中,工作流在推送或向 `master` 分支发起 pull 请求时被触发.它在最新的 Ubuntu 虚拟环境上运行,并设置 Node.js Version为 12,安装依赖,并运行测试.

你可以在仓库in `.github/workflows/` Table of Contents中创建这个 YAML 文件,GitHub Actions 将会自动识别并根据你定义的触发器执行它.
...
CocoaPodsandSwift Package Manager（SPM）都是iOS开发中常用的依赖Management工具,但它们在一些方面上有所不同.下面是两者之间的一些主要区别:

1. **兼容性**:CocoaPods支持Objective-CandSwift,并且它在iOS,macOS,watchOSandtvOS等平台上都能工作.而SPM最初只支持Swift,但在近年来也开始支持Objective-C.至于平台支持,Swift Package Manager 在 Swift 5.3 Version开始支持 iOS, watchOS, and tvOS.
2. **Integration方式**:CocoaPods通过创建一个workspace并将所有的依赖项Integration到一个名为Pods的项目中,然后开发者需要在该workspace中进行开发.而SPM则是直接将依赖项Integration到当前的项目中,开发者可以在项目设置中Management所有的依赖项.
3. **配置**:CocoaPods使用一个名为Podfile的文件来Declare项目的依赖项.而SPM使用Swift语言编写的Package.swift文件来Declare项目的依赖项,这个文件不仅包含了依赖项的信息,还包含了项目的构建配置等信息.
4. **社区支持**:CocoaPods是一个成熟的工具,社区支持广泛.然而,SPM是苹果官方推出的工具,与Xcode的Integration度更高,未来可能会有更多的社区支持.
5. **私有库支持**:CocoaPods可以轻易地配置and使用私有库.SPM在Swift 5.2之后也开始支持私有库,但在配置上可能比CocoaPods要复杂一些.
6. **二进制Framework支持**:CocoaPods支持使用二进制Framework,这可以显著减少编译时间.然而,直到Swift 5.3,SPM才开始支持二进制Framework.
...
4.Auto Layout in Intrinsic Content Size 是干麻用的?
